...
valor=100;
% end
[idx, cMat, sumd] = kmeans(rawInput, cluster, 'Start', seed./25, 'emptyaction','singleton','MaxIter',valor);
...
%   usa un ratio i un m�x de value per la carretera
    ...
    if (cMat(i,2) < 0.45) && (cMat(i,3) > 0.15 && cMat(i,3) < 0.31) && rgbCond
        trashBin = [trashBin, i];
...
if red
    ...
    % sign representation
%     rsBox = [min(signCoords(:,2))+minCol min(signCoords(:,1)) max(signCoords(:,2))-min(signCoords(:,2)) max(signCoords(:,1))-min(signCoords(:,1))];
end
%% Check for trashbin
if trashbin
    if ~isempty(trashBin)
...
if tree
    ...
    for i = 1:length(cornersRectangle)
        k = impoly(handl4, cornersRectangle{i});
    end
    end
end
